...
struct all_ones_sequence {
	...
	inline static uint64_t bitsize(global_parameters const& /* params */,
		...
		// 如果满足universe==n,无需占用bit，否则返回最大值
		return (universe == n) ? 0 : uint64_t(-1);
	}

	template<typename Iterator>
	static void write(succinct::bit_vector_builder&, Iterator,
			uint64_t universe, uint64_t n, global_parameters const&) {
		assert(universe == n);
		(void) universe;
		(void) n;
	}
	...
	public:
		...
		//size函数返回即为m_universe，即默认m_position==m_universe
		{
			assert(universe == n);
			(void) n;
		}
