...
public:
	...
	/*
	 * nvalue: length of units used by compressed data in value of out
	 * 1 indicates an end while 0 indicates a remain
	 */
...
struct optpfor_block {
	...
	struct codec_type: OPTPFor<4, Simple16<false>> {
		...
		// the code
		uint32_t const* decodeBlock(const uint32_t *in, uint32_t *out,
				size_t& nvalue) {
	...
	static uint8_t const* decode(uint8_t const* in, uint32_t* out,
		...
		if (n == block_size) {
			...
			while (out_len < n) {
				...
				uint32_t* bufptr = buf;
				size_t buflen = 8 * 4;
				size_t read = varint_codec.decodeBlock(src, srclen, bufptr,
						buflen);
...
struct interpolative_block {
	...
	static uint8_t const* decode(uint8_t const* in, uint32_t* out,
		...
		out[n - 1] = high;
		if (n > 1) {
			integer_encoding::internals::BitsReader br((uint32_t const*) inbuf,
					2 * n);
