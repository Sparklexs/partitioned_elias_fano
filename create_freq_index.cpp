...
void verify_collection(InputCollection const& input, const char* filename) {
	// InputCollection is binary_freq_collection
	// Collection is freq_index
	Collection coll;
	...
	for (auto seq : input) {
		...
		if (e.size() != seq.docs.size()) {
			logger() << "sequence " << s << " has wrong length! (" << e.size()
					<< " != " << seq.docs.size() << ")" << std::endl;
			exit(1);
		}
		...
		for (size_t i = 0; i < e.size(); ++i, e.next()) {
			...
			if (docid != e.docid()) {
				logger() << "docid in sequence " << s << " differs at position "
						<< i << "!" << std::endl;
			...
			if (freq != e.freq()) {
				logger() << "freq in sequence " << s << " differs at position "
						<< i << "!" << std::endl;
...
void decompress_collection(const char* filename) {
	Collection coll;
	boost::iostreams::mapped_file_source m(filename);
	succinct::mapper::map(coll, m);

//	std::cout << coll.size() << std::endl;
//	std::cout << coll.num_docs() << std::endl;
	size_t s = 0;
	for (size_t s = 0; s < coll.size(); s++) {
		auto e = coll[s];
		for (size_t i = 0; i < e.size(); i++, e.next()) {
			uint64_t docid = e.docid();
			uint64_t freq = e.freq();
		}
	}
}
...
void get_size_stats(
		quasi_succinct::freq_index<DocsSequence, FreqsSequence>& coll,
...
void dump_stats(Collection& coll, std::string const& type, uint64_t postings) {
	...
	quasi_succinct::stats_line()("type", type)("docs_size", docs_size)(
			"freqs_size", freqs_size)("bits_per_doc", bits_per_doc)(
...
void dump_index_specific_stats(quasi_succinct::uniform_index const& coll,
	...
	quasi_succinct::stats_line()("type", type)("log_partition_size",
...
void dump_index_specific_stats(quasi_succinct::opt_index const& coll,
	...
	quasi_succinct::stats_line()("type", type)("eps1", conf.eps1)("eps2",
			conf.eps2)("fix_cost", conf.fix_cost)("docs_avg_part",
			long_postings / docs_partitions)("freqs_avg_part",
			long_postings / freqs_partitions);
}

struct progress_logger {
	progress_logger() :
			sequences(0), postings(0) {
	}

	void log() {
		logger() << "Processed " << sequences << " sequences, " << postings
				<< " postings" << std::endl;
	}
...
void create_collection(InputCollection const& input,
		...
		const char* output_filename, bool check, std::string const& seq_type) {
	using namespace quasi_succinct;

//	{
//		if (check) {
//			double tick = get_time_usecs();
//			decompress_collection<CollectionType>(output_filename);
//			double elapsed_secs = (get_time_usecs() - tick) / 1000000;
//			logger() << "time of decoding: " << elapsed_secs << std::endl;
//		}
//	}
	...
	stats_line()("type", seq_type)("worker_threads",
			configuration::get().worker_threads)("construction_time",
			elapsed_secs)("construction_user_time", user_elapsed_secs);
