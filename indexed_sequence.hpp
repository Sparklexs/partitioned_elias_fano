...
struct indexed_sequence {
	...
	enum index_type {
		elias_fano = 0, ranked_bitvector = 1, all_ones = 2,
	...
	static void write(succinct::bit_vector_builder& bvb, Iterator begin,
		...
		if (best_cost) {
			...
			uint64_t rb_cost = compact_ranked_bitvector::bitsize(params,
					universe, n) + type_bits;
			if (rb_cost < best_cost) {
				best_cost = rb_cost;
	...
	public:
		...
		enumerator(succinct::bit_vector const& bv, uint64_t offset,
			...
			if (all_ones_sequence::bitsize(params, universe, n) == 0) {
				...
				m_type = index_type(
						bv.get_word56(offset)
								& ((uint64_t(1) << type_bits) - 1));
			}

			switch (m_type) {
			case elias_fano:
				m_ef_enumerator = compact_elias_fano::enumerator(bv,
						offset + type_bits, universe, n, params);
				break;
			case ranked_bitvector:
				m_rb_enumerator = compact_ranked_bitvector::enumerator(bv,
						offset + type_bits, universe, n, params);
				break;
			case all_ones:
				m_ao_enumerator = all_ones_sequence::enumerator(bv,
						offset + type_bits, universe, n, params);
				break;
			default:
