...
struct partitioned_sequence {
	...
	static void write(succinct::bit_vector_builder& bvb, Iterator begin,
		...
		optimal_partition opt(begin, universe, n, cost_fun, conf.eps1,
				conf.eps2);
		...
		if (partitions == 1) {
			...
			base_sequence_type::write(bvb, cur_partition.begin(),
					cur_partition.back() + 1, cur_partition.size(), params);
		} else {
			...
			for (size_t p = 0; p < opt.partition.size(); ++p) {
				...
				base_sequence_type::write(bv_sequences, cur_partition.begin(),
						cur_partition.back() + 1, cur_partition.size(), // XXX skip last one?
						params);
			...
			compact_elias_fano::write(bv_sizes, opt.partition.begin(), n,
					partitions - 1, params);
	...
	public:
		...
		enumerator(succinct::bit_vector const& bv, uint64_t offset,
			...
			if (m_partitions == 1) {
				...
				if (n > 1) {
					...
					ub = universe_delta ?
							universe_delta : (universe - m_cur_base - 1);
				}

				m_partition_enum = base_sequence_enumerator(*m_bv,
						it.position(), ub + 1, n, m_params);
				...
				m_sizes = compact_elias_fano::enumerator(bv, cur_offset, n,
						m_partitions - 1, params);
				cur_offset += compact_elias_fano::bitsize(params, n,
						m_partitions - 1);

				m_upper_bounds = compact_elias_fano::enumerator(bv, cur_offset,
						universe, m_partitions + 1, params);
		...
		value_type QS_ALWAYSINLINE move(uint64_t position) {
			...
			if (m_position >= m_cur_begin && m_position < m_cur_end) {
				uint64_t val =
						m_cur_base
								+ m_partition_enum.move(
										m_position - m_cur_begin).second;
	...
	private:
		...
		void switch_partition(uint64_t partition) {
			...
			m_partition_enum = base_sequence_enumerator(*m_bv, partition_begin,
					m_cur_upper_bound - m_cur_base + 1, m_cur_end - m_cur_begin,
					m_params);
		}
